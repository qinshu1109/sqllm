...
type Group struct {
	...
	// 计费模式: balance(余额), subscription(订阅), card(次卡)
	BillingMode string `json:"billing_mode,omitempty"`
	// 次卡模式默认单次价格(USD)
	DefaultCardPrice *float64 `json:"default_card_price,omitempty"`
...
type GroupEdges struct {
	...
	// ModelRates holds the value of the model_rates edge.
...
// ModelRatesOrErr returns the ModelRates value or an error if the edge
// was not loaded in eager-loading.
func (e GroupEdges) ModelRatesOrErr() ([]*GroupModelRate, error) {
	if e.loadedTypes[4] {
		return e.ModelRates, nil
	}
	return nil, &NotLoadedError{edge: "model_rates"}
}
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldDefaultCardPrice:
			values[i] = new(sql.NullFloat64)
		case group.FieldID, group.FieldDefaultValidityDays, group.FieldFallbackGroupID:
			values[i] = new(sql.NullInt64)
		case group.FieldName, group.FieldDescription, group.FieldStatus, group.FieldPlatform, group.FieldSubscriptionType, group.FieldBillingMode:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldDefaultCardPrice:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field default_card_price", values[i])
			} else if value.Valid {
				_m.DefaultCardPrice = new(float64)
				*_m.DefaultCardPrice = value.Float64
			}
		default:
...
// QueryModelRates queries the "model_rates" edge of the Group entity.
func (_m *Group) QueryModelRates() *GroupModelRateQuery {
	return NewGroupClient(_m.config).QueryModelRates(_m)
}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.DefaultCardPrice; v != nil {
		builder.WriteString("default_card_price=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteByte(')')
