...
type GroupQuery struct {
	...
	withModelRates        *GroupModelRateQuery
...
// QueryModelRates chains the current query on the "model_rates" edge.
func (_q *GroupQuery) QueryModelRates() *GroupModelRateQuery {
	query := (&GroupModelRateClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withModelRates:        _q.withModelRates.Clone(),
...
// WithModelRates tells the query-builder to eager-load the nodes that are connected to
// the "model_rates" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithModelRates(opts ...func(*GroupModelRateQuery)) *GroupQuery {
	query := (&GroupModelRateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withModelRates = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withModelRates; query != nil {
		if err := _q.loadModelRates(ctx, query, nodes,
			func(n *Group) { n.Edges.ModelRates = []*GroupModelRate{} },
			func(n *Group, e *GroupModelRate) { n.Edges.ModelRates = append(n.Edges.ModelRates, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadModelRates(ctx context.Context, query *GroupModelRateQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupModelRate)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupmodelrate.FieldGroupID)
	}
	query.Where(predicate.GroupModelRate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.ModelRatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
