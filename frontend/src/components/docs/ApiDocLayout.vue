            ...
            <a
              ...
              :class="[
                ...
                activeSection === item.id
                  ? 'text-primary-500 dark:text-primary-400 font-medium border-l-2 border-primary-500 -ml-[2px] pl-[calc(0.75rem+2px)]'
...
function setupScrollSpy() {
  if (!props.tocItems?.length) return
  ...
  const sectionIds = props.tocItems.map(item => item.id)
  ...
  observer = new IntersectionObserver(
    ...
    {
      rootMargin: '-80px 0px -60% 0px', // 顶部留出导航栏空间
      threshold: 0
    }
  )
